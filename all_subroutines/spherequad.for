...
c
c     ==================================================================
      subroutine readintpoints(filepath,nmax,intrule,n,a,b,x,y,z,rhow)
c     ==================================================================
c     PURPOSE:
c     This subroutine reads the integration points from file.
c     
c     NOTES:
c     ------------------------------------------------------------------
c     INPUT:
c       filepath: Path of the integration points text file.
c       nmax:     Maximum number of integration points (for array 
c                 initialization).
c     OUTPUT:
c       n:     Number of integration points.
c       a:     Von Mises function in-plane concentration parameter.
c       b:     Von Mises function out-of-plane concentration parameter.
c       x:     x coordinate integration points [-].
c       y:     y coordinate integration points [-].
c       z:     y coordinate integration points [-].
c       rhow:  Integrand value rho*w at integration points.
c     ------------------------------------------------------------------
c     AUTHOR:                                                 R.Alberini
...
c     ------------------------------------------------------------------
      ...
      integer n
      real*8 a,b
      real*8, dimension(nmax) :: x,y,z,w,rho,rhow
      ! Other variables.
      integer i,j,p,nrows,io,istart
      character*256 dummy,intrule
      real*8, dimension(nmax) :: az,el
      real*8 pi,temp(8)
      parameter( pi=3.1415926535897932d0 )
      ! Error message variables. Use Abaqus utility routine stdb_abqerr
      ! to print the error message to the *.msg file.
      character*500 errormsg,nmaxchar
      integer intv(10)
      real*8 realv(10)
...
c     ------------------------------------------------------------------
      ! Open file.
      open(199, file = trim(filepath), status = 'old', action = 'read')
      ! Find number of rows.
      nrows = 0
      do
        read(199,'(A)',iostat=io) dummy
        nrows = nrows + 1
        if (io.ne.0) then
          if (io.eq.-1) then
            ! End of file reached
...
c     ------------------------------------------------------------------
c     READ INTEGRATION POINTS INFORMATION
c     ------------------------------------------------------------------
      ! Rewind file (read from the beginning).
      rewind(199)
      
      ! Read information.
      read(199,'(A)') dummy              ! Row 1: creator of the file.
      read(199,'(A)') dummy              ! Row 2: empty.
      read(199,'(A)') dummy              ! Row 2: headings.
      read(199,'(A)') dummy              ! Row 2: rules.
      read(199,'(A6,F10.2)') dummy,a     ! Row 3: ip concentration.
      read(199,'(A6,F10.2)') dummy,b     ! Row 4: op concentration.
      read(199,'(A6,A10)') dummy,intrule ! Row 5: integration rule.
      
      ! Justify inegration rule to the left.
      intrule = adjustl(intrule)
...
c     ------------------------------------------------------------------
      ...
      ! Find points start command row.
      do i=1,nrows
        read(199,'(A)') dummy
        if ( dummy(1:12).eq.'Points start' ) then
          ! Define starting row and exit loop.
          istart = i+2 ! Plus 2 to skip headings after 'Points start'.
          exit
        end if
        
        ! Terminate analysis if no points start command is detected.
        if ( i.eq.nrows ) then
          close(199)
          errormsg = 'INTEGRATION POINTS STARTING ROW HAS NOT BEEN'//
     &               ' DETECTED IN '//trim(filepath)
          call stdb_abqerr(-3,errormsg,intv,realv,charv)
          call xit
        end if
      end do
c     ------------------------------------------------------------------
c     READ INTEGRATION POINTS
c     ------------------------------------------------------------------
      ! Rewind file (read from the beginning).
      rewind(199)
      
      ! Skip rows before the first point.
      do i=1,istart-1
        read(199,'(A)') dummy
      end do
      ...
      j = 0
      do i=istart,nrows
        ! Initialize variables every loop iteration.
        p = 0
        temp = 0d0
        
        ! Read row.
        read(199,'(I6,5F16.6,3E16.6)',iostat=io) p,temp
        
        ! Exit loop if the row does not contain an integration point or
        ! if a reading error is encountered (io=0 successful reading,
        ! io=-1 end of file).
        if ( (p.eq.0).and.all(temp.eq.0d0) ) exit
        if ( (io.ne.0).and.(io.ne.-1) ) exit
        
        ! Exit loop and terminate analysis if number of integration
        ! points exceeds max number of points <nmax>.
        j = j+1
        if (j.gt.nmax) then
          close(199)
          write(nmaxchar,'(I10)') nmax
          errormsg = 'INTEGRATION POINTS IN '//trim(filepath)//
     &               ' EXCEED MAXIMUM NUMBER OF POINTS '//
     &               trim(adjustl(nmaxchar))//'. '//
     &               'REDUCE NUMBER OF POINTS OR CHANGE MAXIMUM'//
     &               ' NUMBER IN KPARS_UMAT.FOR'
          call stdb_abqerr(-3,errormsg,intv,realv,charv)
          call xit
        end if
      ...
      ! Update number of integration points.
      n = j
      
      ! Close file.
      close(199)
      
      ! Terminate analysis if there are no points.
      if ( j.eq.0 ) then
        errormsg = 'NO INTEGRATION POINTS FOUND IN '//trim(filepath)
        call stdb_abqerr(-3,errormsg,intv,realv,charv)
        call xit
      end if
c
      return
      end subroutine readintpoints
c     ==================================================================
c
c
c     ==================================================================
      subroutine lebedevintegr(n,a,b,x,y,z,w,rho)
c     ==================================================================
c     PURPOSE:
c     This subroutine returns the list of integration points and weights
c     over the unit sphere computed with the Lebedev-Laikov quadrature.
c     First, one of the subroutines LD#### is called to compute the 
c     Lebedev integration points (in rectangular coordinates) and the 
c     corresponding weights on the unit sphere. Then, the von Mises 
c     distribution is evaluated at every Lebedev point.
c
c     NOTES:
c     ------------------------------------------------------------------
c     INPUT/OUTPUT:
c       n:     Target number of integration points on the unit sphere.
c              The number is updated during the execution.
c     INPUT:
c       a:     Von Mises function in-plane concentration parameter.
c       b:     Von Mises function out-of-plane concentration parameter.
c     OUTPUT:
c       x:     x coordinate of Lebedev points [-].
c       y:     y coordinate of Lebedev points [-].
c       z:     y coordinate of Lebedev points [-].
c       w:     Weights of Lebedev points (sum is 1)
c       rho:   Values of the von Mises distribution at Lebedev point.
c     ------------------------------------------------------------------
c     AUTHOR:                                                 R.Alberini
c     LAST MODIFIED:                                          25/06/2024
c     ==================================================================
c     ------------------------------------------------------------------
c     DECLARATION OF VARIABLES
c     ------------------------------------------------------------------
      ...
      real*8 :: a,b
      ! Output.
      real*8, dimension(5810) :: x,y,z,w,rho
      ! Input/Output.
      integer :: n
      ! Other variables.
      integer :: i,m,nleb(29)
      real*8 :: pi,vm_nonsymm
      real*8, dimension(5810) :: azimuth,elevation,r,az,el
      parameter( pi=3.1415926535897932d0 )
c     ------------------------------------------------------------------
c     COMPUTE LEBEDEV POINTS AND WHEIGHTS
c     ------------------------------------------------------------------
      ! List of possible number of Lebedev integration points on the
      ! full unit sphere (removed 74, 230, 266 from the list because
      ! some integration points in these sets have negative weights).
      nleb = (/ 6, 14, 26, 38, 50, 86, 110, 146, 170, 194, 302, 350,
     & 434, 590, 770, 974, 1202, 1454, 1730, 2030, 2354, 2702, 3074,
     & 3470, 3890, 4334, 4802, 5294, 5810 /)
      ! Pick the number of Lebedev integration points closer to n.
      m = nleb(minloc(abs(nleb-n),1))
      ! Address the computation to the subroutine LD####.
      if (m.eq.6) then
        call LD0006(x,y,z,w,n)
      else if (m.eq.14) then
        call LD0014(x,y,z,w,n)
      else if (m.eq.26) then
        call LD0026(x,y,z,w,n)
      else if (m.eq.38) then
        call LD0038(x,y,z,w,n)
      else if (m.eq.50) then
        call LD0050(x,y,z,w,n)
      else if (m.eq.74) then
        call LD0074(x,y,z,w,n)
      else if (m.eq.86) then
        call LD0086(x,y,z,w,n)
      else if (m.eq.110) then
        call LD0110(x,y,z,w,n)
      else if (m.eq.146) then
        call LD0146(x,y,z,w,n)
      else if (m.eq.170) then
        call LD0170(x,y,z,w,n)
      else if (m.eq.194) then
        call LD0194(x,y,z,w,n)
      else if (m.eq.230) then
        call LD0230(x,y,z,w,n)
      else if (m.eq.266) then
        call LD0266(x,y,z,w,n)
      else if (m.eq.302) then
        call LD0302(x,y,z,w,n)
      else if (m.eq.350) then
        call LD0350(x,y,z,w,n)
      else if (m.eq.434) then
        call LD0434(x,y,z,w,n)
      else if (m.eq.590) then
        call LD0590(x,y,z,w,n)
      else if (m.eq.770) then
        call LD0770(x,y,z,w,n)
      else if (m.eq.974) then
        call LD0974(x,y,z,w,n)
      else if (m.eq.1202) then
        call LD1202(x,y,z,w,n)
      else if (m.eq.1454) then
        call LD1454(x,y,z,w,n)
      else if (m.eq.1730) then
        call LD1730(x,y,z,w,n)
      else if (m.eq.2030) then
        call LD2030(x,y,z,w,n)
      else if (m.eq.2354) then
        call LD2354(x,y,z,w,n)
      else if (m.eq.2702) then
        call LD2702(x,y,z,w,n)
      else if (m.eq.3074) then
        call LD3074(x,y,z,w,n)
      else if (m.eq.3470) then
        call LD3470(x,y,z,w,n)
      else if (m.eq.3890) then
        call LD3890(x,y,z,w,n)
      else if (m.eq.4334) then
        call LD4334(x,y,z,w,n)
      else if (m.eq.4802) then
        call LD4802(x,y,z,w,n)
      else if (m.eq.5294) then
        call LD5294(x,y,z,w,n)
      else if (m.eq.5810) then
        call LD5810(x,y,z,w,n)
      end if
c     ------------------------------------------------------------------
c     COMPUTE VON MISES DISTIBUTION VALUES AT INTEGRATION POINTS
c     ------------------------------------------------------------------
      ! Transform cartesian coordinates in spherical coordinates.
      do i=1,n
        azimuth(i) = atan2(y(i),x(i))
        elevation(i) = atan2(z(i),sqrt(x(i)**2 + y(i)**2))
        r(i) = sqrt(x(i)**2 + y(i)**2 + z(i)**2)
      end do
      ! Compute von Mises distribution at integration points.
      do i=1,n
        az(i)  = azimuth(i)*180d0/pi         ! In-plane angle.
        el(i)  = elevation(i)*180d0/pi       ! Out-of-plane angle.
        rho(i) = vm_nonsymm(a,b,az(i),el(i)) ! Von Mises probability.
      end do
c
      return
      end subroutine lebedevintegr
c     ==================================================================
c
c
c     ==================================================================
      function vm_nonsymm(a,b,az,el)
c     ==================================================================
c     PURPOSE:
c     Non-symmetrical von Mises probability density function.
c
c     NOTES:
c     ------------------------------------------------------------------
c     INPUT:
c       a:   In-plane concentration parameter (-inf,inf).
c       b:   Out-of-plane concentration parameter (-inf,inf).
c       az:  Azimuth angle [deg].
c       el:  Elevation angle [deg].
c     OUTPUT:
c       vm_nonsymm: rho(az,el) = rho_ip(az)*rho_op(el)
c     ------------------------------------------------------------------
c     AUTHOR:                                                 R.Alberini
c     LAST MODIFIED:                                          30/06/2022
c     ==================================================================
c     ------------------------------------------------------------------
c     DECLARATION OF VARIABLES
c     ------------------------------------------------------------------
      ...
      ! Other variables.
      integer n,i
      real*8 pi,rho_ip,rho_op,besI0,normf
      complex*16 cbi0,cdi0,cbi1,cdi1,cbk0,cdk0,cbk1,cdk1,errf
      complex*16 za,zb
      parameter( pi=3.1415926535897932d0 )
c     ------------------------------------------------------------------
c     COMPUTE THE PROBABILITY DENSITY FUNCTION
c     ------------------------------------------------------------------
      ! Compute in-plane probability rho_ip.
      za = cmplx(a,0d0)
      call cik01(za,cbi0,cdi0,cbi1,cdi1,cbk0,cdk0,cbk1,cdk1)
      besI0 = real(cbi0)
      rho_ip = exp(a*cos(2d0*az*pi/180d0))/besI0
      ! Compute out-of-plane probability rho_op.
      zb = cmplx(b,0d0)
      !print *, 'culo ',i,real(sqrt(2d0*zb)),' +',aimag(sqrt(2d0*zb)),'i'
      call erfcomplex(sqrt(2d0*zb),errf)
      !print *, 'culo ',i,real(errf),' +',aimag(errf),'i'
      normf = real(2d0*sqrt(2d0*b/cmplx(pi,0d0))/errf)
      rho_op = exp(b*(cos(2d0*el*pi/180d0)-1d0))*normf
      ! Compute total probability.
      vm_nonsymm = rho_ip*rho_op
c
      return
      end function vm_nonsymm
c     ==================================================================
c
c
c     ==================================================================
...
c
c     ==================================================================
      subroutine erfcomplex ( z, cer )
      ! Subroutine name changed from "cerror" to "erfcomplex" to avoid
      ! the following Abaqus error in log file:
      !
...
c     ==================================================================
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
c      C-code [1] was kindly provided by Dr. Dmitri N. Laikov and 
c      translated into fortran by Dr. Christoph van Wuellen.
c      This subroutine was translated from C to fortran77 by hand.
c   
c      Users of this code are asked to include reference [1] in their
c      publications, and in the user- and programmers-manuals 
c      describing their codes.
c   
c      This code was distributed through CCL (http://www.ccl.net/).
c   
...
c   
c      [2] V.I. Lebedev
c          "A quadrature formula for the sphere of 59th algebraic
c           order of accuracy"
c          Russian Acad. Sci. Dokl. Math., Vol. 50, 1995, pp. 283-286. 
c   
c      [3] V.I. Lebedev, and A.L. Skorokhodov
c          "Quadrature formulas of orders 41, 47, and 53 for the sphere"
c          Russian Acad. Sci. Dokl. Math., Vol. 45, 1992, pp. 587-592. 
c   
c      [4] V.I. Lebedev
c          "Spherical quadrature formulas exact to orders 25-29"
c          Siberian Mathematical Journal, Vol. 18, 1977, pp. 99-107. 
c   
c      [5] V.I. Lebedev
c          "Quadratures on a sphere"
c          Computational Mathematics and Mathematical Physics, Vol. 16,
c          1976, pp. 10-24. 
c   
c      [6] V.I. Lebedev
c          "Values of the nodes and weights of ninth to seventeenth 
c           order Gauss-Markov quadrature formulae invariant under the
c           octahedron group with inversion"
c          Computational Mathematics and Mathematical Physics, Vol. 15,
c          1975, pp. 44-51.
c   
c  
c      Given a point on a sphere (specified by a and b), generate all
c      the equivalent points under Oh symmetry, making grid points with
c      weight v.
c      The variable num is increased by the number of different points
c      generated.
c  
c      Depending on code, there are 6...48 different but equivalent
c      points.
c  
...
c  
    6  continue
       ...
       double precision A,B,V
c  
c      LEBEDEV    6-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
c      C-code [1] was kindly provided by Dr. Dmitri N. Laikov and 
c      translated into fortran by Dr. Christoph van Wuellen.
c      This subroutine was translated using a C to fortran77 conversion
c      tool written by Dr. Christoph van Wuellen.
c   
c      Users of this code are asked to include reference [1] in their
c      publications, and in the user- and programmers-manuals 
c      describing their codes.
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV   14-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV   26-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV   38-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV   50-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV   74-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV   86-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  110-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  146-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  170-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  194-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  230-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  266-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  302-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  350-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  434-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  590-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  770-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV  974-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 1202-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 1454-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 1730-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 2030-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 2354-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 2702-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 3074-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 3470-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 3890-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 4334-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 4802-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 5294-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
...
c   
       ...
       double precision A,B,V
c  
c      LEBEDEV 5810-POINT ANGULAR GRID
c  
c   
c      This subroutine is part of a set of subroutines that generate
c      Lebedev grids [1-6] for integration on a sphere. The original 
