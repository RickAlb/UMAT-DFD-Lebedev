...
## Purpose
Abaqus-Standard subroutine based on the Discrete Fibers Dispersion (DFD) model in finite strains (Ehret et al. 2010, Li et al., 2018) for fibre-reinforced soft biological materials.
...
- uniform prestress field $`\sigma_0`$ applicable to all elements in the model (the components of the relative deformation gradient $`\mathrm{F}_0`$ must be provided in the prestretch file).

The model is implemented applying the rotation correction to the deformation gradient $`\mathrm{F}`$ when a local orientation {$`\mathrm{E}_1`$, $`\mathrm{E}_2`$, $`\mathrm{E}_3`$} is specified in conjunction with Solid (Continuum) elements. See Nolan et al. (2022) for further details.

Includes the subroutine for the Lebedev integration points computation (Lebedev & Laikov, 1999).

Special functions subroutines are also included under Shanjie Zhang and Jianming Jin permission (Zhang & Jin, 1996).
...
To allow the subroutine to be properly compiled, open the environment file `abaqus_v6.env` located in the Abaqus installation folder and add the following flags to the fortran compiler settings:

- Linux platform (GCC compiler): ` -fcray-pointer`.
  * For example `compile_fortran = (fortCmd + " -free" + " -fcray-pointer" + " -c -fPIC -I%I")`
- Windows platform (Intel Fortran Compiler):
...
- `HB` nearly incompressible formulation with decoupled deviatoric and volumetric strain-energies. The deviatoric fiber strain-energy (DFD model) is implemented using the full deformation gradient according to Helfenstein et al. (2010) and Nolan et al. (2014).
...
- `IN`:
  ...
  * 2D membrane (Structural) elements `M3D3`, `M3D4`, ``
...
| ------------- | ------------- | ------------- | ------------- |
| $`1`$ | `bulk` | [x]Pa | Bulk modulus (ignored in incompressible formulation, `IN`). |
| $`2`$ | `νg` | - | Ground matrix volume fraction. |
| $`3`$ | `μ` | [x]Pa | Ground matrix shear modulus. |
| $`4`$ | `c1` | [x]Pa | Fibers stiffness-like parameter. |
...
| $`6n+6`$ | `rint` |  | Integration scheme flag (`0` use implemented Lebedev quadrature, `999` read points from external file(s)). |
...
| $`6n+8`$ | `thr` |  | Threshold value for integration points retention. Points with integrand value lower than `thr` will be discarded (`1e-6` suggested, `0` keeps all). |
| $`6n+9`$ | `csw` |  | Fibers compression switch flag (`1` exclude compressed fibers, `0` include all fibers). |
| $`6n+10`$ | `elem` |  | Element type (`1` Continuum, `2` Structural). |
| $`6n+11`$ | `ori` |  | Type of active orientation (`0` global orientation, {$`\mathrm{G}_1`$, $`\mathrm{G}_2`$, $`\mathrm{G}_3`$} basis, `1` local orientation, {$`\mathrm{E}_1`$, $`\mathrm{E}_2`$, $`\mathrm{E}_3`$} basis). |
...
- The material must include at least one fiber family;
- The sum of all the fiber families volume fractions `νfi` should be equal to 1;
- The local basis rotation angles `θ1`, `θ2`, `θ3`, are used to print the deformation gradient $`\mathrm{F}`$ and the Cauchy stress tensor $`\sigma`$ components relative to the global basis {$`\mathrm{G}_1`$, $`\mathrm{G}_2`$, $`\mathrm{G}_3`$} on a `.chy` file in the analysis' folder. The target element and the relative Gauss point number must be specified in the `kpars_umat.for` source file. 
...
```
abaqus job=input_file_name user=main_sub cpus=n
```

where `main_sub` is the main subroutine `main_sub.for` including `UMAT_DFD_LEB.for` and all the subroutines required for the analysis. The subroutine supports parallel computing. Change `n` to define the desired number of cores.
